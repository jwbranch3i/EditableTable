...
import editableTable.EditCell;
import editableTable.MyDateStringConverter;
import editableTable.MyDoubleStringConverter;
...
public class TableAppController implements Initializable
{
	...
	private void setupDateOfBirthColumn()
	{
		...
		dateOfBirthColumn.setOnEditCommit(event ->
		{
			...
			((PersonTableData) event.getTableView().getItems().get(event.getTablePosition().getRow())).setDateOfBirth(value);
			table.refresh();
		});
	}
	...
	{
		salaryColumn.setCellFactory(EditCell.<PersonTableData, Double>forTableColumn(new MyDoubleStringConverter()));
